...
if (developmentChains.includes(network.name)) {
  describe("Gas Price Compare Tests", function () {
    ...
    describe("Base NFT Tests", function () {
      it("(PUBLIC_MINT) Should mint multiples NFTs", async function () {
        ...
        await collectibleNFT.connect(addrs[0]).publicSingleMint_Eq({
            value: ethers.BigNumber.from(`${PRICE * MAX_MINTS_PER_USER}`),
          });

        const nftOwnerBalance = await collectibleNFT.balanceOf(addrs[0].address);
        expect(nftOwnerBalance).to.equal(ethers.BigNumber.from("1"));

        await collectibleNFT.connect(addrs[1]).publicSingleMint_Eq({
            value: ethers.BigNumber.from(`${PRICE * MAX_MINTS_PER_USER}`),
          });

        const nftOwnerBalance2 = await collectibleNFT.balanceOf(addrs[1].address);
        expect(nftOwnerBalance2).to.equal(ethers.BigNumber.from("1"));

        await collectibleNFT.connect(addrs[2]).publicSingleMint_Eq({
            value: ethers.BigNumber.from(`${PRICE * MAX_MINTS_PER_USER}`),
          });

        const nftOwnerBalance3 = await collectibleNFT.balanceOf(addrs[2].address);
        expect(nftOwnerBalance3).to.equal(ethers.BigNumber.from("1"));

        await collectibleNFT.connect(addrs[3]).publicSingleMint_Eq({
            value: ethers.BigNumber.from(`${PRICE * MAX_MINTS_PER_USER}`),
          });

        const nftOwnerBalance4 = await collectibleNFT.balanceOf(addrs[3].address);
        expect(nftOwnerBalance4).to.equal(ethers.BigNumber.from("1"));

        await collectibleNFT.connect(addrs[4]).publicSingleMint_Eq({
            value: ethers.BigNumber.from(`${PRICE * MAX_MINTS_PER_USER}`),
          });

        const nftOwnerBalance5 = await collectibleNFT.balanceOf(addrs[4].address);
        expect(nftOwnerBalance5).to.equal(ethers.BigNumber.from("1"));
      });
    });
  });
} else {
